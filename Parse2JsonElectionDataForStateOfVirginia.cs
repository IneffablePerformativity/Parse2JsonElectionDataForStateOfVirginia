...
using Newtonsoft.Json; // V. .net 4.5

namespace Parse2JsonElectionDataForStateOfVirginia
{
	...
	class Program
	{
		static string ProjectName = "Parse2JsonElectionDataForStateOfVirginia";

		static string LocationBeingStudied = "StateOfVirginia";

		static string LocationBeingStudiedClearly = "State Of Virginia";

		static string gitHubRepositoryURL = "https://github.com/IneffablePerformativity/" + ProjectName;

		static string gitHubRepositoryShortened = "https://bit.ly/3qlvQZg";
...
//		static int howOrdering = 8; // 8 is net bonus
		...
		// Remember "totalBallots" is a misnomer, an approximation,
		// It is really my summing up of all the grain POTUS votes.
		...
		static string inputDir = @"C:\A\SharpDevelop\" + ProjectName + @"\inputFiles";
		...
		// zero, one, or two
		const string contestSenus1 = "Member United States Senate";
		...
		const string contestRepus1 = "Member House of Representatives (01)";
		const string contestRepus2 = "Member House of Representatives (02)";
		const string contestRepus3 = "Member House of Representatives (03)";
		const string contestRepus4 = "Member House of Representatives (04)";
		const string contestRepus5 = "Member House of Representatives (05)";
		const string contestRepus6 = "Member House of Representatives (06)";
		const string contestRepus7 = "Member House of Representatives (07)";
		const string contestRepus8 = "Member House of Representatives (08)";
		const string contestRepus9 = "Member House of Representatives (09)";
		const string contestRepus10 = "Member House of Representatives (10)";
		const string contestRepus11 = "Member House of Representatives (11)";
...
//			else
		...
		// VIRGINIA looks like a no-fraud graph, but 1% clutters plot.
		static int XPct = 2;
		...
		static void doit()
		{
			...
			Input2JsonFiles(); // new format for Virginia
		...
		// here is the tail that will wag the dog:

		
		static void Input2JsonFiles()
		{
			...
			string[] inputFilePaths = Directory.GetFiles(inputDir, "*.json");
			// 133 here -- say("Qty files = " + inputFilePaths.Length);
			foreach(string inputFilePath in inputFilePaths)
			{
				string inputFileName = Path.GetFileName(inputFilePath);
				// say(inputFileName);

				// The files are JSON fragments with a dict as top object.
				// Must prefix '{"topData":' and suffix '}' to turn that
				// JSON fragment into a proper JSON document.
				
				string rawJson = File.ReadAllText(inputFilePath);
				TopObject topObject = JsonConvert.DeserializeObject<TopObject>(rawJson);
				
				if(topObject == null)
					throw(new Exception("topObject == null"));
				
				// say(topObject.ElectionName); // baby step 2.
				// say(topObject.ElectionDate);
				// say(topObject.CreateDate);
				if(topObject.Locality == null)
				{
					// This is the case of Presidential or Congress summary files.
					say("topObject.Locality == null");
					continue;
				}
				else
				{
					// This is the case of by-county files.
					
					// say(topObject.Locality.LocalityName);
					// Good. I have grain (name).
					grain = topObject.Locality.LocalityName;
					// Only one I saw is not all UPPER + Spaces:
					// -- KING & QUEEN COUNTY
					// say(topObject.Locality.LocalityCode);
				}

				// say("topObject.Races.Length = " + topObject.Races.Length);
				for(int iRace = 0; iRace < topObject.Races.Length; iRace++)
				{
					// say("RaceName[" + iRace + "] = " + topObject.Races[iRace].RaceName);
					// collect just this list of strings to put back into switch() cases:
					// say(topObject.Races[iRace].RaceName);

					contest = topObject.Races[iRace].RaceName;
					// There's plenty of minor races.
					// I only want POTUS, SENUS, REPUS:
					//President and Vice President
					//Member United States Senate
					//Member House of Representatives (02)
					//123456789 123456789 12345
					if(contest.Length < 25)
						continue;
					switch(contest.Substring(0, 25))
					{
						case "President and Vice Presid":
							break;
						case "Member United States Sena":
							break;
						case "Member House of Represent":
							break;
						default:
							continue;
					...
					// say(contest);
					// Good. I have all the contest names to re-import into const for switch()s
					// 133 President and Vice President
					// 133 Member United States Senate
					//  29 Member House of Representatives (09)
					//  23 Member House of Representatives (05)
					//  20 Member House of Representatives (01)
					//  19 Member House of Representatives (06)
					//  16 Member House of Representatives (04)
					//  10 Member House of Representatives (07)
					//  10 Member House of Representatives (02)
					//   8 Member House of Representatives (10)
					//   8 Member House of Representatives (03)
					//   4 Member House of Representatives (08)
					//   3 Member House of Representatives (11)

					// say("PrecinctsReporting[" + iRace + "] = " + topObject.Races[iRace].PrecinctsReporting);
					// say("PrecinctsParticipating[" + iRace + "] = " + topObject.Races[iRace].PrecinctsParticipating);
					// say("NumberOfSeats[" + iRace + "] = " + topObject.Races[iRace].NumberOfSeats);
					// say("topObject.Races[" + iRace + "].Candidates.Length = " + topObject.Races[iRace].Candidates.Length);
					for(int iCandidate = 0; iCandidate < topObject.Races[iRace].Candidates.Length; iCandidate++)
					{
						// say("topObject.Races[" + iRace + "].Candidates[" + iCandidate + "].BallotName = " + topObject.Races[iRace].Candidates[iCandidate].BallotName);
						// say("topObject.Races[" + iRace + "].Candidates[" + iCandidate + "].BallotOrder = " + topObject.Races[iRace].Candidates[iCandidate].BallotOrder);
						
						// say("topObject.Races[" + iRace + "].Candidates[" + iCandidate + "].Votes = " + topObject.Races[iRace].Candidates[iCandidate].Votes);
						
						// say(topObject.Races[iRace].Candidates[iCandidate].Votes.ToString());
						// Good. I have the votes.
						votes = topObject.Races[iRace].Candidates[iCandidate].Votes.ToString();
						
						// say("topObject.Races[" + iRace + "].Candidates[" + iCandidate + "].Percentage = " + topObject.Races[iRace].Candidates[iCandidate].Percentage);
						// say("topObject.Races[" + iRace + "].Candidates[" + iCandidate + "].PoliticalParty = " + topObject.Races[iRace].Candidates[iCandidate].PoliticalParty);

						// Now let's examine the candidates
						// say(topObject.Races[iRace].Candidates[iCandidate].BallotName);
						// Good. I have choices within contests.
						choice = topObject.Races[iRace].Candidates[iCandidate].BallotName;

						// say(topObject.Races[iRace].Candidates[iCandidate].PoliticalParty);
						// Good I have parties, to distill into REP/DEM/ETC:
						//Democratic
						//Republican
						//Libertarian
						//Write-In
						...
						party = "ETC";
						switch(topObject.Races[iRace].Candidates[iCandidate].PoliticalParty)
						{
							case "Democratic":
								party = "DEM";
								break;
						...
						// say("pondering: " + contest + choice + party + grain + votes);
	...
	public class TopObject
	{
		...
		public LocalityITem Locality;
		public RaceItem [] Races;
	}
	...
	public class RaceItem
	{
		...
		public int PrecinctsReporting;
		public int PrecinctsParticipating;
